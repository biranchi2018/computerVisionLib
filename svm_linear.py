...
for imagePath in paths.list_images(args["training"]):
        ...
        print("Train: {} for this image {}".format(make, imagePath))
        #----以下為訓練圖檔的預處理----# 
        #載入圖檔,轉為灰階,作模糊化處理
        image = cv2.imread(imagePath)
        #gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        moment.loadImage(objImage=image, resize_w=500)
        #將裁切後的圖檔尺寸更改為500x500。
        #Cutted = cv2.resize(gray, (500, 500))
        cv2.imshow("Training: "+make, image)
#----訓練圖檔預處理結束----#
        #取得其HOG資訊及視覺化圖檔
        ...
        H = moment.HuFeatures()
        print("Number: {} , moments--> {}".format(make, H))
        #H = moment.HuFeatures1(colorSpace="RGB", channelSelect=(0,0,1), blur=11, threshold=(120,140,150), bwInvert=True, debug=False)
        data.append(H)
        labels.append(make)
...
#----訓練圖檔預處理結束----#
        ...
        print("Predict number: {} , moments--> {}".format(pred, H))
        #顯示HOG視覺化圖檔
